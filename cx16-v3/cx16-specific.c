...
/* For remote AI I/O */
#define CHT_EOL		'\0'
#define VIA2_PRTA	(*(volatile unsigned char *)(0x9F10))
...
void UpdateDisplay(void)
{
	...
	/* Save the frame */
	FRAME_CMD = 1;
	/* Dummy code to prevent "not used" warnings */
	s_frame_clock = current_clock = clock();
#endif /* REMOTE_AI */
...
#ifdef REMOTE_AI
/* Convenience method to output numbers to the remote AI without linking in the entire stdio library! */
	...
	if (c && c != UCHAR_MAX) {
		do {
			switch (c) {
				...
				case 'f':
				case 'g':
					ReportToAI(player, EVT_HUMAN_JOINED, MD_JOIN_AS_NORMAL + c - 'e');
					ResetMessages();
					break;
				case 'a':
					player->z_delta = (player->last_z_delta > 0)? 2: 1;
				...
				case 'd':
					...
					break;
				case 'l':
					player->a_delta = (player->last_a_delta < 0)? -2: -1;	
				...
				case 'r':
					...
					break;
				case 'x':
					/* No particular target, note falling through to next case */
					c = '0' + PLAYER_LIMIT - PLAYER_INDEX;
				case '1':
				case '2':
				case '3':
					if (!player->loading_cd) {
						player->firing = true;
						player->target = PLAYER_INDEX + c - '0';
					}
					break;
			}		
		} while (c = VIA2_PRTA);
