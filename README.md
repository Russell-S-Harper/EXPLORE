...
For those who want to try out the compiled program, you can copy the three files beginning with `PVP-AI` from my [Google Drive](https://drive.google.com/drive/folders/1nBoKQ6ZB6Fb3NYO77jeoF8aATl1pe7U-?usp=sharing) and run the `PRG`. Suggest running in an emulator (R49) for now because there is a problem with the VERA hardware, see the note below.
...
Inspired by other attempts to have LLMs interact with 8-bit systems, most notably [ChatGPT vs Atari 2600 Video Chess](https://www.linkedin.com/posts/robert-jr-caruso-23080180_ai-chess-atari2600-activity-7337108175185145856-HSP0), I wanted to investigate what would be required to have a successful interaction between an LLM and a simple game. It turns out that PvP-AI is an ideal test candidate:

- access to the internal state of the game means that information can be provided to the LLM not otherwise accessible
- novel approaches to level advancement and winning conditions make the game compatible to “alternate strategies” one might encounter when integrating with AI
- the emulator, which PvP-AI runs on, is open source, and can be modified if required
...
V3 introduces several accommodations to make the game compatible with the LLM:
...
For rapid experimentation, the LLM interface layer was built in PHP, which interacts directly with the ChatGPT API (model: *gpt-4o*) and coordinates state exchange with the emulator. Due to IP considerations and that a key component is still under review, this repository includes:
...
Until the pull request is finalized, I would recommend not trying to build this yet. However, the process would be identical to that for `cx16-v2`. A functioning build directory with complete PHP and JSON can be provided for review under appropriate terms and conditions.
